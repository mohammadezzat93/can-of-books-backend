...
server.put('/updateBooks/:id', updateBookHandeler);
...
// http://localhost:3010/updateBooks
async function updateBookHandeler(req, res) {
  const bookId = req.params.id;
  const { bookName, description,status, email } = req.body;

  await BookModel.findByIdAndUpdate(bookId, { title: bookName, description: description, status:status }, (err, result1) => {
  })
    .catch(err => {
      console.log(err);
    })
